    ...
    var basicBot = {
        ...
        commands: {
	       ...
	       autorouletteCommand: {
		    ...
		    autorouletteppCommand: {
                command: 'autoroletapp',
                rank: 'bouncer',
                type: 'exact',
                functionality: function (chat, cmd) {
                    if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
                    if (!basicBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (basicBot.settings.autoroulettepp) {
                            basicBot.settings.autoroulettepp = !basicBot.settings.autoroulettepp;
                            return API.sendChat(subChat(basicBot.chat.toggleoff, {name: chat.un, 'function': basicBot.chat.autoroulettepp}));
                        }
                        else {
                            basicBot.settings.autoroulettepp = !basicBot.settings.autoroulettepp;
                            return API.sendChat(subChat(basicBot.chat.toggleon, {name: chat.un, 'function': basicBot.chat.autoroulettepp}));
                        }
